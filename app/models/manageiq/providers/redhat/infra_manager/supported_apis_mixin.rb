module ManageIQ::Providers::Redhat::InfraManager::SupportedApisMixin
  def supported_api_versions(directly_probe_the_provider = false)
    return supported_api_versions_from_cache if api_version.blank? || directly_probe_the_provider
  ...
  def supported_api_versions_from_cache
    ...
    cacher.fetch_fresh(last_refresh_date, :force => force) { supported_api_versions_from_sdk_through_queue }
  end

  def self.supported_api_versions_from_sdk(args)
  ...
  def supported_api_versions_from_sdk_through_queue
    ...
    queue_options = {
      ...
      :method_name => 'supported_api_versions_from_sdk',
      :zone        => my_zone,
      :args        => args
    }
