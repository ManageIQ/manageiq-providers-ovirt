class ManageIQ::Providers::Ovirt::InfraManager::Inventory
  ...
  def collect_snapshots_total_size(vms)
    vms.each do |vm|
      ManageIQ::Providers::Ovirt::Inventory::Collector.add_snapshot_disks_total_size(connection, vm.snapshots, vm.id)
    end
  end
  ...
  def collect_inv_with_attributes_async(inv, inv_type)
    ...
    inv_fetched_attributes = ManageIQ::Providers::Ovirt::InfraManager::FuturesCollector.process_keyed_requests_queue(inv_keyed_requests)
