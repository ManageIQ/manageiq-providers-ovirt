class ManageIQ::Providers::Ovirt::Inventory::Collector < ManageIQ::Providers::Inventory::Collector
  ...
  def collect_attached_disks(disks_owner)
    manager.with_provider_connection do |connection|
      ManageIQ::Providers::Ovirt::InfraManager::Inventory::DisksHelper.collect_attached_disks(disks_owner, connection, preloaded_disks)
    end
  end
  ...
  def collect_disks_as_hash
    manager.with_provider_connection do |connection|
      ManageIQ::Providers::Ovirt::InfraManager::Inventory::DisksHelper.collect_disks_as_hash(connection)
    end
  end
  ...
  def self.add_snapshot_disks_total_size(connection, snapshots, vm_id)
    snapshots.each do |snapshot|
      snapshot.extend(ManageIQ::Providers::Ovirt::InfraManager::SnapshotDisksMixin)
