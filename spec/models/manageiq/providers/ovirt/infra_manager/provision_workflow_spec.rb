describe ManageIQ::Providers::Ovirt::InfraManager::ProvisionWorkflow do
  ...
  include_examples "check class for platform", "ovirt"

  it "pass platform attributes to automate" do
    assert_automate_dialog_lookup(admin, "infra", "ovirt", "get_pre_dialog_name", nil)
  ...
  context "allowed clusters" do
    let(:workflow) { described_class.new({:src_vm_id => template.id}, admin) }
    let(:datacenter1) { FactoryBot.create(:ems_folder, :type => "ManageIQ::Providers::Ovirt::InfraManager::Datacenter") }
    let(:datacenter2) { FactoryBot.create(:ems_folder, :type => "ManageIQ::Providers::Ovirt::InfraManager::Datacenter") }
    ...
    let(:template) { FactoryBot.create(:template_ovirt, :ext_management_system => ems, :ems_cluster => cluster1) }
