describe VmScan do
  context "A single VM Scan Job," do
    let(:server_vm) { FactoryBot.create(:vm_ovirt, :host => FactoryBot.create(:host)) }
    let!(:server) do
    ...
    context "#call_check_policy" do
      it "should raise vm_scan_start for Vm" do
        expect(MiqAeEvent).to receive(:raise_evm_event).with(
          "vm_scan_start",
          an_instance_of(ManageIQ::Providers::Ovirt::InfraManager::Vm),
          an_instance_of(Hash),
          an_instance_of(Hash)
        )
      ...
      it "should raise vm_scan_start for template" do
        ...
        expect(MiqAeEvent).to receive(:raise_evm_event).with(
          "vm_scan_start",
          an_instance_of(ManageIQ::Providers::Ovirt::InfraManager::Template),
          an_instance_of(Hash),
          an_instance_of(Hash)
        )
        job.call_check_policy
