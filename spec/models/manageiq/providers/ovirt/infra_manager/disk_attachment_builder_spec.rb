describe ManageIQ::Providers::Ovirt::InfraManager::DiskAttachmentBuilder do
  context "#disk_format_for" do
    context "when storage type is file system" do
      let(:storage) { FactoryBot.build(:storage_nfs) }
    ...
    context "when storage type is block" do
      let(:storage) { FactoryBot.build(:storage_block) }
    ...
    context "when storage type is not file system and not blcok" do
      let(:storage) { FactoryBot.build(:storage_unknown) }
  ...
  context "#disk_attachment" do
    let(:storage) { FactoryBot.build(:storage_nfs, :ems_ref => "http://example.com/storages/XYZ") }
  ...
  describe ManageIQ::Providers::Ovirt::InfraManager::DiskAttachmentBuilder::BooleanParameter do
    let(:param) { nil }
