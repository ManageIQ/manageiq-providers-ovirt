  ...
  context "A new provision request," do
    ...
    context "RHEV-M provisioning" do
      before(:each) do
        ...
        @vm_template = FactoryBot.create(:template_ovirt, :name => "template1", :ext_management_system => @ems, :operating_system => @os, :cpu_limit => -1, :cpu_reserve => 0)
        @vm          = FactoryBot.create(:vm_ovirt, :name => "vm1", :location => "abc/def.vmx")
        ...
        @vm_prov = FactoryBot.create(:miq_provision_ovirt, :userid => @admin.userid, :miq_request => @pr, :source => @vm_template, :request_type => 'template', :state => 'pending', :status => 'Ok', :options => @options)
      end

      it "#workflow" do
        workflow_class = ManageIQ::Providers::Ovirt::InfraManager::ProvisionWorkflow
      ...
      context "with a destination vm" do
        let(:destination_vm) { FactoryBot.create(:vm_ovirt, :ext_management_system => @ems) }
        let(:state) { "down" }
        before do
          stub_settings_merge(
            :ems => {
