describe ManageIQ::Providers::Ovirt::InfraManager::Provision::Configuration do
  ...
  let(:host)          { FactoryBot.create(:host_ovirt) }
  let(:task)          { FactoryBot.create(:miq_provision_ovirt, :state => 'pending', :status => 'Ok', :options => {:src_vm_id => template.id}) }
  ...
  before { allow_any_instance_of(ManageIQ::Providers::Ovirt::InfraManager::Provision).to receive(:with_provider_destination).and_yield(provider_object) }
  context "#attach_floppy_payload" do
    context "with ovirtsdk4" do
      ...
      let(:ovirt_services)  { double("ManageIQ::Providers::Ovirt::InfraManager::OvirtServices::V4") }
      let(:provider_object) { ManageIQ::Providers::Ovirt::InfraManager::OvirtServices::V4::VmProxyDecorator.new(vm_service, connection, ovirt_services) }
