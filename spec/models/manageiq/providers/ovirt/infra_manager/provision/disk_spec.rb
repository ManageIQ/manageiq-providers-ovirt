describe ManageIQ::Providers::Ovirt::InfraManager::Provision::Disk do
  ...
  let(:rhevm_vm)      { FactoryBot.build(:vm_ovirt) }
  let(:vm)            { FactoryBot.build(:vm_ovirt, :ext_management_system => ems) }
  ...
  before do
    @task = FactoryBot.build(:miq_provision_ovirt,
                              :source      => template,
                              :destination => vm,
                              :state       => 'pending',
                              :status      => 'Ok',
