describe ManageIQ::Providers::Ovirt::InfraManager::Vm::Reconfigure do
  let(:ems) do
    _, _, zone = EvmSpecHelper.create_guid_miq_server_zone
    FactoryBot.create(:ems_ovirt, :zone => zone)
  end
  let(:storage) { FactoryBot.create(:storage_nfs, :ems_ref => "http://example.com/storages/XYZ") }
  let(:vm)      { FactoryBot.create(:vm_ovirt, :storage => storage) }

  describe "#reconfigurable?" do
    let(:vm_active)   { FactoryBot.create(:vm_ovirt, :storage => storage, :ext_management_system => ems) }
    let(:vm_retired)  { FactoryBot.create(:vm_ovirt, :retired => true, :storage => storage, :ext_management_system => ems) }
    let(:vm_archived) { FactoryBot.create(:vm_ovirt) }
  ...
  context "#build_config_spec" do
    ...
    let(:vm) { FactoryBot.create(:vm_ovirt, :hardware => FactoryBot.create(:hardware), :storage => storage) }
    ...
    context 'network adapters spec' do
      ...
      let(:host1) { FactoryBot.create(:host_ovirt, :ext_management_system => ems, :ems_cluster => cluster1) }
      let(:host2) { FactoryBot.create(:host_ovirt, :ext_management_system => ems, :ems_cluster => cluster1) }
      let!(:distributed_virtual_switch) { FactoryBot.create(:distributed_virtual_switch_ovirt, :ems_id => ems.id, :name => "network") }
      ...
      shared_examples_for 'not available lan' do |network_name|
        ...
        it 'no lan for vm host' do
          wrong_switch = FactoryBot.create(:distributed_virtual_switch_ovirt, :ems_id => ems.id, :name => "wrong_network")
      ...
      context 'external lan' do
        let!(:dc2) { FactoryBot.create(:datacenter_ovirt, :name => 'dc2', :ems_ref => 'dc2-ems-ref', :ems_ref_type => 'Datacenter') }
  ...
  describe 'available_vlans' do
    ...
    let(:switch1) { FactoryBot.create(:distributed_virtual_switch_ovirt, :ems_id => ems.id, :name => "network1") }
    let(:switch2) { FactoryBot.create(:distributed_virtual_switch_ovirt, :ems_id => ems.id, :name => "network2") }
    ...
    let(:vm) { FactoryBot.create(:vm_ovirt, :ext_management_system => ems, :host => host1, :storage => storage) }
