  ...
  context "A new provision request," do
    ...
    context "RHEV-M provisioning" do
      before(:each) do
        ...
        @vm_prov = FactoryBot.create(:miq_provision_redhat_via_iso, :userid => @admin.userid, :miq_request => @pr, :source => @vm_template, :request_type => 'template', :state => 'pending', :status => 'Ok', :options => @options)
      end
