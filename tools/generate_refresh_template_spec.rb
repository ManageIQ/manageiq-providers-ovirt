describe(ManageIQ::Providers::Redhat::InfraManager::Refresh::Refresher) do
  ...
  def relations_tree_to_a(expected_tree)
    ...
    expected_tree.each do |obj, children|
      key = [wrap_name_for_inspect(obj.class.name), obj.name] # [obj.class.name, obj.name]
  ...
  it("will generate a new cassete") do
    VCR.use_cassette("#{described_class.name.underscore}_generated_from_v3", :allow_unused_http_interactions => true, :allow_playback_repeats => true, :record => :new_episodes) do
      EmsRefresh.refresh(@ems)
    end
    @ems.reload

    file_name = ManageIQ::Providers::Ovirt::Engine.root.join('/spec/models/manageiq/providers/redhat/infra_manager/refresh/refresher_spec_generator_template.txt')
    output_file_name = ManageIQ::Providers::Ovirt::Engine.root.join('/spec/models/manageiq/providers/redhat/infra_manager/refresh/refresher_4_async_spec.rb')
